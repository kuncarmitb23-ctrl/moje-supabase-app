...
import { Search, Cpu, HardDrive, Monitor, MemoryStick, ShoppingCart, ArrowLeft, CheckCircle } from 'lucide-react';
// TADY IMPORTUJEME TVŮJ KOŠÍK (uprav cestu podle potřeby)
...
const Bazar: React.FC = () => {
  ...
  // Vytáhneme si z tvého Contextu jen to, co potřebujeme (věci v košíku a funkci na přidání)
  ...
  // Funkce, která převede náš Supabase produkt do formátu, který čeká tvůj CartContext
  const handlePridatDoKosiku = (e: React.MouseEvent, produkt: Produkt) => {
    e.stopPropagation();
    addToCart({
      id: String(produkt.id), // Tvůj context má id jako string
      name: produkt.nazev,
      price: produkt.cena,
      image: produkt.obrazek_url,
      // DŮLEŽITÉ: Přidáme si i originální ID ze Supabase, 
      // abychom pak při objednávce věděli, co zneviditelnit!
      supabaseId: produkt.id 
    });
  };
          ...
          <div className="max-w-6xl mx-auto animate-in fade-in duration-300">
            ...
            <div className="bg-zinc-900 border border-white/5 rounded-3xl overflow-hidden flex flex-col md:flex-row shadow-2xl relative">
              ...
              <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col">
                ...
                {/* TLAČÍTKO V DETAILU */}
                {items.some(i => i.id === String(vybranyProdukt.id)) ? (
                  <button disabled className="w-full bg-zinc-800 text-zinc-500 py-5 rounded-2xl font-black flex items-center justify-center gap-3 uppercase tracking-widest cursor-not-allowed">
                    <CheckCircle size={24} /> Již v košíku
                  </button>
                ) : (
                  <button 
                    onClick={(e) => {
                      handlePridatDoKosiku(e, vybranyProdukt);
                      setVybranyProdukt(null); // Zavřeme detail, ať může vybírat dál
                    }}
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {vyfiltrovaneProdukty.map((produkt) => {
                  ...
                  // Ověříme v Contextu, zda už tam produkt je (porovnáváme jako string)
                  const jeVKosiku = items.some(item => item.id === String(produkt.id));
